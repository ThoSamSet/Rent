    ...
    <nav className="booking-wizard-progress" aria-label="Tiến trình đặt lịch">
      <ol className="booking-wizard-progress__list">
        {BOOKING_WIZARD_STEPS.map((step) => (
          <li key={step.id} className="booking-wizard-progress__item" data-wizard-progress-step={step.id}>
            <span className="booking-wizard-progress__no">{String(step.id).padStart(2, '0')}</span>
